...
func (srv *Server) parseBlueskyURL(ctx context.Context, raw string) (*syntax.ATURI, error) {
	...
	if u.Hostname() != "app.bsky.transgirl.fr" {
		return nil, fmt.Errorf("only app.bsky.transgirl.fr URLs currently supported")
	}
	...
	if len(pathParts) != 5 || pathParts[1] != "profile" || pathParts[3] != "post" {
		return nil, fmt.Errorf("only app.bsky.transgirl.fr post URLs currently supported")
	}
...
func (srv *Server) WebOEmbed(c echo.Context) error {
	...
	if err != nil {
		return c.String(http.StatusBadRequest, fmt.Sprintf("Expected 'url' to be app.bsky.transgirl.fr URL or AT-URI: %v", err))
	}
	...
	data := OEmbedResponse{
		...
		AuthorURL:    fmt.Sprintf("https://app.bsky.transgirl.fr/profile/%s", post.Author.Handle),
